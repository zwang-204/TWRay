...
bool bvh_node::hit(const Ray& r, float t_min, float t_max, SurfaceInteraction& si) const {

    if (!box.IntersectP(r, &t_min, &t_max))
        return false;

    bool hit_left = left->hit(r, t_min, t_max, si);
    bool hit_right = right->hit(r, t_min, t_max, si);
