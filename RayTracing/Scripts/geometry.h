...
Vector3<float> Reflect(const Vector3f& v, const Normal3f& n){
    return v - 2*Dot(v,Vector3f(n)) * Vector3f(n);
}

Vector3<float> Refract(const Vector3f &uv, const Normal3f &n, float etai_over_etat){
...
Vector3<float> Random_in_unit_disk(){
    while(true) {
...
Vector3<float> Random_in_unit_sphere() {
    while(true) {
