...
bool sphere::hit(const Ray& r, float t_min, float t_max, SurfaceInteraction& si) const {
    ...
    si.time = root;
    si.p = r(si.time);
    Normal3f outward_normal = Normal3f((si.p - center) / radius);
    si.set_face_normal(r, outward_normal);
    get_sphere_uv(Vector3f(outward_normal), si.uv[0], si.uv[1]);
    si.mat_ptr = mat_ptr;

    return true;
}

bool sphere::bounding_box(Bounds3f& output_box) const {
    output_box = Bounds3f(
...
float sphere::pdf_value(const Point3f& o, const Vector3f& v) const {
    ...
    if(!this->hit(Ray(o, v), 0.001, Infinity, si))
        return 0;
